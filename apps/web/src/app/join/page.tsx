...
function JoinForm() {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!companyId || !inviteId) return setError("This invitation link is incomplete. Ask your Director for a new link."); setSaving(true); setError(""); try { let credential; try { credential = await createUserWithEmailAndPassword(auth, email.trim(), password); } catch (authError) { if ((authError as { code?: string }).code !== "auth/email-already-in-use") throw authError; credential = await signInWithEmailAndPassword(auth, email.trim(), password); } const inviteRef = doc(db, "companies", companyId, "invites", inviteId); const invite = await getDoc(inviteRef); if (!invite.exists() || !invite.data().active || String(invite.data().email).toLowerCase() !== email.trim().toLowerCase()) throw new Error("invalid-invite"); const batch = writeBatch(db); batch.set(doc(db, "companies", companyId, "users", credential.user.uid), { companyId, name: name.trim(), email: email.trim().toLowerCase(), role: invite.data().role, inviteId, assignedProjectIds: [], active: true, createdAt: serverTimestamp() }); batch.set(doc(db, "userIndex", credential.user.uid), { companyId, createdAt: serverTimestamp() }); batch.update(inviteRef, { active: false, acceptedBy: credential.user.uid, acceptedAt: serverTimestamp() }); await batch.commit(); router.replace("/"); } catch { setError("We could not join the workspace. Use the invited email and the password chosen on your first attempt, then try again."); } finally { setSaving(false); } };
