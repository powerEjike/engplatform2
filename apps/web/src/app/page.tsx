...
function NotificationBell() {
  return <svg className="notification-bell" viewBox="0 0 24 24" aria-hidden="true"><path d="M18 9a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9M10 21h4" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8" /></svg>;
}
    ...
    <div className="app-shell">
      ...
      <main id="top">
        <header className="topbar">
          ...
          <div className="profile"><Link className="dashboard-notifications" href="/updates" aria-label={`View workspace updates${attentionActions.length ? `, ${attentionActions.length} item${attentionActions.length === 1 ? "" : "s"} need attention` : ""}`}><NotificationBell /><b>Updates</b>{attentionActions.length > 0 && <em>{Math.min(attentionActions.length, 9)}</em>}</Link><span className="avatar">{profile.name.split(" ").map((part) => part[0]).join("").slice(0, 2).toUpperCase()}</span><span className="role-label"><i aria-hidden="true" />{profile.role.replaceAll("_", " ")}</span><button className="sign-out" type="button" onClick={() => void signOutUser()}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M10 5H5v14h5M14 8l4 4-4 4M18 12H9" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8" /></svg><span>Sign out</span></button></div><MobileDashboardMenu items={dashboardMenuItems} name={profile.name} role={profile.role} companyName={profile.companyName} onSignOut={() => void signOutUser()} />
        </header>
