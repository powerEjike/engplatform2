    ...
    <div className="app-shell">
      ...
      <main id="top">
        <header className="topbar">
          ...
          <div className="profile"><Link className="dashboard-notifications" href="/updates" aria-label={`View workspace updates${attentionActions.length ? `, ${attentionActions.length} item${attentionActions.length === 1 ? "" : "s"} need attention` : ""}`}><NotificationBell /><b>Updates</b>{attentionActions.length > 0 && <em>{Math.min(attentionActions.length, 9)}</em>}</Link><span className="avatar">{profile.name.split(" ").map((part) => part[0]).join("").slice(0, 2).toUpperCase()}</span><span className="role-label">Role: {profile.role.replace("_", " ")}</span><button className="sign-out" type="button" onClick={() => void signOutUser()}>Sign out</button></div><MobileDashboardMenu items={dashboardMenuItems} name={profile.name} role={profile.role} companyName={profile.companyName} onSignOut={() => void signOutUser()} />
        </header>
