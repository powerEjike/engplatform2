...
export default function ProjectWorkspacePage() {
  ...
  return <main className="project-workspace"><div className="workspace-content">
    ...
    <section className="boq-card"><div className="boq-header"><div><p className="eyebrow">Cost plan</p><h2>BOQ items</h2></div><p className="boq-total">Planned BOQ value<strong>{formatNaira(plannedValue)}</strong></p></div><p className="field-hint">Visible to the Director, Quantity Surveyor, Project Manager, and the assigned Site Engineer. Only the Director and Quantity Surveyor can upload or edit the BOQ.</p>{canManageBoq(profile.role) && items.length > 0 && <button className="clear-boq-button" type="button" disabled={isClearingBoq} onClick={() => void clearBoq()}>{isClearingBoq ? "Removing BOQ…" : "Remove all unused BOQ items"}</button>}{error && <p className="form-error">{error}</p>}{items.length === 0 ? <p className="boq-empty">No BOQ items yet. Start with the major contract work items, such as preliminaries, foundation, structure, finishes, or services.</p> : <div className="boq-list">{items.map((item) => { const progress = item.plannedQuantity > 0 ? Math.min(100, Math.round(item.cumulativeQuantityCompleted / item.plannedQuantity * 100)) : 0; return <article className="boq-item" key={item.id}><div className="boq-item-top"><div><span className="boq-item-number">{item.section} · {item.itemNumber}</span><h3>{item.description}</h3></div><strong>{formatNaira(item.plannedQuantity * item.rate)}</strong></div><p className="boq-item-meta">{item.plannedQuantity.toLocaleString()} {item.unit} × {formatNaira(item.rate)} per {item.unit}</p><div className="boq-item-progress"><div><span>{item.cumulativeQuantityCompleted.toLocaleString()} of {item.plannedQuantity.toLocaleString()} {item.unit} complete</span><strong>{progress}%</strong></div><div className="progress-track"><span style={{ width: `${progress}%` }} /></div></div>{canManageBoq(profile.role) && <button className="remove-boq-button" type="button" onClick={() => void removeBoqItem(item)}>Remove item</button>}</article>; })}</div>}</section>
    </div>
