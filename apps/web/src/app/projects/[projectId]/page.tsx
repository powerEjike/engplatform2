...
import { canFinalApproveVariation, canGenerateValuation, canManageBoq, canRaiseVariation, canReviewVariation, canSubmitReport } from "@/lib/permissions";
...
export default function ProjectWorkspacePage() {
  ...
  return <main className="project-workspace"><div className="workspace-content">
    ...
    <section className={`site-assignment ${project.siteEngineerName ? "assigned" : "unassigned"}`}><div><p className="eyebrow">Project team</p><h2>{project.projectManagerName ? `Project Manager: ${project.projectManagerName}` : "Project Manager not assigned"}</h2><p>{project.siteEngineerName ? `Site Engineer: ${project.siteEngineerName} · responsible for daily reports and site-level variations.` : "Assign a Site Engineer before site reporting begins."}</p></div>{profile.role === "director" ? <Link className="secondary compact-action" href={`/projects/${projectId}/settings`}>Manage team</Link> : <span className="assignment-status">{project.siteEngineerName ? "Site assigned" : "Awaiting assignment"}</span>}</section>
