...
export default function ProjectWorkspacePage() {
  ...
  return <main className="project-workspace"><div className="workspace-content">
    ...
    <div className="project-title-row"><div><p className="eyebrow">Project BOQ</p><h1>{project.name}</h1><p>{project.clientName} · {project.location}, {project.state}</p></div><div className="project-title-actions">{canSubmitReport(profile.role) && <Link className="primary-action" href={`/projects/${projectId}/reports/new`}>+ Daily site report</Link>}{canRaiseVariation(profile.role) && <Link className="secondary compact-action" href={`/projects/${projectId}/variations/new`}>Raise variation</Link>}{canGenerateValuation(profile.role) && <Link className="secondary compact-action" href={`/projects/${projectId}/valuations/new`}>Create valuation</Link>}{canManageProject(profile.role) && <Link className="text-action" href={`/projects/${projectId}/settings`}>Project settings</Link>}<div className="contract-summary"><span>Contract sum</span><strong>{formatNaira(project.contractSum)}</strong></div></div></div>
