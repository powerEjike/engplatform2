...
export default function NewSiteReportPage() {
  ...
  const saveReport = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      batch.set(report, { projectId, submittedBy: user.uid, reportDate, status: "synced", lineItems, labourCount: labour, equipmentOnSite: [], issues: issue.trim() ? [{ category: "other", note: issue.trim() }] : [], photoIds: [], clientGeneratedId: report.id, createdAt: serverTimestamp(), syncedAt: serverTimestamp() });
