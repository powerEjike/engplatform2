...
export default function NewSiteReportPage() {
  ...
  const saveReport = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      batch.set(report, { projectId, submittedBy: user.uid, reportDate, status: "synced", reviewStatus: "submitted", lineItems, labourCount: labour, labourByTrade: labourBreakdown, equipmentOnSite, equipmentHours: equipmentBreakdown, issues: issue.trim() ? [{ category: issueCategory, note: issue.trim() }] : [], photoIds: [], clientGeneratedId, createdAt: serverTimestamp(), syncedAt: serverTimestamp() });
  ...
  if (!canWorkOnProject(profile.role, user.uid, project)) return <main className="auth-loading">This report page is only available for your assigned projects. <Link href="/dashboard">Return to your assigned projects</Link></main>;
