...
function milestoneTiming(milestone: ProjectMilestone) {
  const planned = new Date(`${milestone.plannedDate}T00:00:00`).getTime();
  const reference = milestone.status === "complete" && milestone.actualDate ? new Date(`${milestone.actualDate}T00:00:00`).getTime() : new Date(new Date().toISOString().slice(0, 10)).getTime();
  const days = Math.round((reference - planned) / 86_400_000);
  if (milestone.status === "complete") return days > 0 ? { label: `${days} day${days === 1 ? "" : "s"} late`, tone: "late" } : days < 0 ? { label: `${Math.abs(days)} day${Math.abs(days) === 1 ? "" : "s"} early`, tone: "early" } : { label: "Completed on plan", tone: "early" };
  if (days > 0) return { label: `${days} day${days === 1 ? "" : "s"} overdue`, tone: "late" };
  if (days === 0) return { label: "Due today", tone: "due" };
  return { label: `Due in ${Math.abs(days)} day${Math.abs(days) === 1 ? "" : "s"}`, tone: "upcoming" };
}
