...
export default function NewProjectPage() {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try { await addDoc(collection(db, "companies", profile.companyId, "projects"), { name: name.trim(), clientName: clientName.trim(), contractSum: value, location: location.trim(), state, startDate, endDate, status: "active", projectManagerId: profile.role === "project_manager" ? user.uid : null, projectManagerName: profile.role === "project_manager" ? profile.name : null, createdBy: user.uid, createdAt: serverTimestamp() }); router.replace("/"); }
