...
export default function ReportsPage() {
  ...
  return <main className="report-page"><div className="schedule-content"><Link className="back-link" href="/">← Back to workspace</Link><section className="schedule-hero"><div><p className="eyebrow">Operations control</p><h1>Daily reports</h1><p>Review site activity, labour, completed BOQ items, and recorded issues across your projects.</p></div><div className="schedule-hero-metrics"><span><strong>{todayReports}</strong> submitted today</span><span><strong>{reportedIssues}</strong> recorded issues</span></div></section><section className="reports-portfolio-list">{reports.length === 0 ? <p className="empty-state">No daily reports have been submitted yet.</p> : reports.map((report) => <article className="reports-portfolio-row" key={`${report.projectId}-${report.id}`}><div><span className="report-history-date">{new Date(`${report.reportDate}T00:00:00`).toLocaleDateString("en-NG", { day: "numeric", month: "short", year: "numeric" })}</span><Link className="project-name-link" href={`/projects/${report.projectId}`}><h2>{report.projectName}</h2></Link></div><p><span>Completed items</span><strong>{report.lineItems.length}</strong></p><p><span>Labour on site</span><strong>{report.labourCount}</strong></p><p className="reports-portfolio-issue">{report.issues[0] ? <><b>{report.issues[0].category.replaceAll("_", " ")}</b> · {report.issues[0].note}</> : "No issue recorded"}</p></article>)}</section></div></main>;
}
