...
export default function ReportsPage() {
  ...
  const downloadReports = () => {
    ...
    const rows = ["Report date,Project,Completed BOQ items,Labour on site,Equipment,Issues", ...reports.map((report) => [report.reportDate, report.projectName, report.lineItems.length, report.labourCount, report.equipmentOnSite.join("; "), report.issues.map((issue) => `${issue.category.replaceAll("_", " ")}: ${issue.note}`).join("; ")].map(quote).join(","))];
