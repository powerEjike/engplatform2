...
const daysSince = (date: string) => {
  ...
  const reportDay = new Date(`${date}T00:00:00`);
...
export default function ReportsPage() {
  ...
  const overdueProjects = visibleProjects.filter((project) => {
    if (project.status !== "active") return false;
    const latestReport = reportsByProject[project.id]?.[0];
    return !latestReport || daysSince(latestReport.reportDate) >= 3;
  });
