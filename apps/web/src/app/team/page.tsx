...
export default function TeamPage() {
  ...
  const toggleAccountAccess = async (memberId: string, isActive: boolean, memberName: string) => {
    if (!profile || memberId === user?.uid) return;
    const action = isActive ? "disable" : "restore";
    if (!window.confirm(`${action === "disable" ? "Disable" : "Restore"} workspace access for ${memberName}?`)) return;
    setSavingId(memberId); setError(""); setMessage("");
    try {
      await updateDoc(doc(db, "companies", profile.companyId, "users", memberId), { active: !isActive });
  ...
  return <main className="report-page"><div className="report-content"><Link className="back-link" href="/dashboard">← Back to workspace</Link><section className="report-card"><p className="eyebrow">Company administration</p><h1>Team management</h1><p className="report-intro">Set each team member&apos;s working role. Client accounts are not available in this workspace.</p><p className="report-intro"><Link className="text-action" href="/team/invite">Invite a team member →</Link></p><div className="role-guide">{roles.map((role) => <p key={role.value}><strong>{role.label}</strong><span>{role.description}</span></p>)}</div><section className="report-section"><h2>Open invitations</h2>{invites.length === 0 ? <p className="field-hint">No invitation links have been created yet.</p> : invites.filter((invite) => invite.active).map((invite) => <p key={invite.id} className="field-hint"><strong>{invite.email}</strong> · invited as {roles.find((role) => role.value === invite.role)?.label ?? invite.role}</p>)}</section><section className="team-management-list">{users.map((member) => { const isCurrentUser = member.id === user.uid; const isLastActiveDirector = member.role === "director" && member.active && activeDirectors.length === 1; const managedProjects = projects.filter((project) => project.projectManagerId === member.id); const assignedProjects = projects.filter((project) => project.siteEngineerId === member.id); const allocationText = member.role === "project_manager" ? `${managedProjects.length} managed project${managedProjects.length === 1 ? "" : "s"}${managedProjects.length ? ` · ${managedProjects.map((project) => project.name).join(", ")}` : ""}` : member.role === "site_engineer" ? `${assignedProjects.length} site assignment${assignedProjects.length === 1 ? "" : "s"}${assignedProjects.length ? ` · ${assignedProjects.map((project) => project.name).join(", ")}` : ""}` : member.role === "quantity_surveyor" ? "Controls BOQ, variations, and valuations across the workspace" : "Portfolio oversight and final approvals"; return <article className={`team-management-row ${member.active ? "" : "account-disabled"}`} key={member.id}><div><h2>{member.name}</h2><p>{member.email}</p></div><label>Role<select value={member.role} disabled={isCurrentUser || savingId === member.id} onChange={(event) => void updateRole(member.id, event.target.value)}>{roles.map((role) => <option key={role.value} value={role.value}>{role.label}</option>)}</select></label>{!isCurrentUser && <button className="secondary compact-action" type="button" disabled={savingId === member.id || isLastActiveDirector} onClick={() => void toggleAccountAccess(member.id, member.active, member.name)}>{member.active ? "Disable access" : "Restore access"}</button>}<span className="role-change-note">{isCurrentUser ? "Your own Director role is protected" : isLastActiveDirector ? "Keep at least one active Director" : savingId === member.id ? "Saving…" : member.active ? allocationText : "Workspace access is disabled"}</span></article>; })}</section>{message && <p className="form-success">{message}</p>}{error && <p className="form-error">{error}</p>}</section></div></main>;
}
