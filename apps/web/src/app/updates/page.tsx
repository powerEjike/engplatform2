...
type UpdateItem = { id: string; title: string; detail: string; date: string; href: string; kind: "report" | "variation" | "valuation" | "boq" };
...
export default function UpdatesPage() {
  ...
  }, [eventsByProject, profile, projects, reportsByProject, user, valuationsByProject, variationsByProject]);
  ...
  return <main className="report-page"><div className="schedule-content"><Link className="back-link" href="/">← Back to workspace</Link><section className="schedule-hero"><div><p className="eyebrow">Live notifications</p><h1>Workspace updates</h1><p>Recent BOQ uploads, reports, variations, and valuation activity from the projects available to your role.</p></div><div className="schedule-hero-metrics"><span><strong>{updates.length}</strong> recent update{updates.length === 1 ? "" : "s"}</span></div></section><section className="updates-list">{updates.length === 0 ? <p className="empty-state">There are no updates for your role yet. New site activity will appear here automatically.</p> : updates.map((update) => <Link className="update-row" href={update.href} key={update.id}><span className={`update-icon ${update.kind}`}>{update.kind === "report" ? "Report" : update.kind === "variation" ? "Variation" : update.kind === "boq" ? "BOQ" : "Valuation"}</span><div><h2>{update.title}</h2><p>{update.detail}</p></div><time dateTime={update.date}>{new Date(update.date).toLocaleDateString("en-NG", { day: "numeric", month: "short", year: "numeric" })}</time></Link>)}</section></div></main>;
}
