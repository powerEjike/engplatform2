...
export default function UpdatesPage() {
  ...
  const updates = useMemo(() => {
    ...
    accessibleProjects.forEach((project) => {
      ...
      if (canSeeVariations) (variationsByProject[project.id] ?? []).forEach((variation) => items.push({ id: `variation-${project.id}-${variation.id}`, kind: "variation", title: `Variation ${variation.status.replaceAll("_", " ")}`, detail: `${project.name} · ${variation.description}`, date: updateDate(variation.approvedAt || variation.reviewedAt || variation.raisedAt), href: `/projects/${project.id}` }));
      if (canSeeValuations) (valuationsByProject[project.id] ?? []).forEach((valuation) => items.push({ id: `valuation-${project.id}-${valuation.id}`, kind: "valuation", title: `Valuation ${valuation.status}`, detail: `${project.name} · ${valuation.certificateNumber}`, date: updateDate(valuation.createdAt) || valuation.valuationDate, href: `/projects/${project.id}` }));
  ...
  return <main className="report-page"><div className="schedule-content"><Link className="back-link" href="/dashboard">← Back to workspace</Link><section className="schedule-hero"><div><p className="eyebrow">Live notifications</p><h1>Workspace updates</h1><p>Recent project assignments, BOQ uploads, reports, variations, valuations, and milestone activity from the projects available to your role.</p></div><div className="schedule-hero-metrics"><span><strong>{updates.length}</strong> recent update{updates.length === 1 ? "" : "s"}</span></div></section><section className="updates-list">{updates.length === 0 ? <p className="empty-state">There are no updates for your role yet. New site activity will appear here automatically.</p> : updates.map((update) => <Link className="update-row" href={update.href} key={update.id}><span className={`update-icon ${update.kind}`}>{update.kind === "project" ? "Project" : update.kind === "report" ? "Report" : update.kind === "variation" ? "Variation" : update.kind === "boq" ? "BOQ" : update.kind === "milestone" ? "Milestone" : "Valuation"}</span><div><h2>{update.title}</h2><p>{update.detail}</p></div><time dateTime={update.date}>{new Date(update.date).toLocaleDateString("en-NG", { day: "numeric", month: "short", year: "numeric" })}</time></Link>)}</section></div></main>;
}
