...
export default function ValuationsPage() {
  ...
  return <main className="report-page"><div className="schedule-content"><Link className="back-link" href="/">← Back to workspace</Link><section className="schedule-hero"><div><p className="eyebrow">Commercial control</p><h1>Valuation register</h1><p>Every issued payment certificate across your available projects.</p></div><div className="schedule-hero-metrics"><span><strong>{certificates.length}</strong> certificate{certificates.length === 1 ? "" : "s"}</span><span><strong>{formatNaira(totalDue)}</strong> total net due</span></div></section>{certificates.length > 0 && <div className="report-export-row"><button className="secondary compact-action" type="button" onClick={downloadValuations}>Download CSV</button></div>}<section className="valuation-portfolio-table"><div className="valuation-portfolio-head"><span>Certificate</span><span>Project</span><span>Date</span><span>Gross value</span><span>Net due</span></div>{certificates.length === 0 ? <p className="empty-state">No valuations have been issued yet.</p> : certificates.map((certificate) => <article className="valuation-portfolio-row" key={`${certificate.projectId}-${certificate.id}`}><div><span className="valuation-certificate">{certificate.certificateNumber}</span><p>{certificate.status}</p></div><Link className="project-name-link" href={`/projects/${certificate.projectId}`}>{certificate.projectName}</Link><span>{new Date(`${certificate.valuationDate}T00:00:00`).toLocaleDateString("en-NG", { day: "numeric", month: "short", year: "numeric" })}</span><strong>{formatNaira(certificate.grossValue)}</strong><strong className="valuation-net-value">{formatNaira(certificate.netAmountDue)}</strong></article>)}</section></div></main>;
}
