...
export default function VariationsPage() {
  ...
  const variations = useMemo(() => {
    ...
    return projects.filter((project) => canWorkOnProject(profile.role, user.uid, project)).flatMap((project) => (variationsByProject[project.id] ?? []).map((variation) => ({ ...variation, projectName: project.name }))).filter((variation) => (selectedStatus === "All statuses" || variation.status === selectedStatus) && (selectedProject === "All projects" || variation.projectId === selectedProject)).sort((left, right) => String(right.approvedAt || right.reviewedAt || right.raisedAt).localeCompare(String(left.approvedAt || left.reviewedAt || left.raisedAt)));
  }, [profile, projects, selectedProject, selectedStatus, user, variationsByProject]);
