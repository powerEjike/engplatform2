  ...
  return <div className="mobile-dashboard-menu">
    ...
    {isOpen && <>
      ...
      <aside className="mobile-menu-panel" id="mobile-dashboard-navigation" aria-label="Dashboard menu">
        ...
        <nav className="mobile-menu-links" aria-label="Mobile dashboard navigation">
          <Link className="mobile-menu-home" href="/dashboard" onClick={() => setIsOpen(false)}><span aria-hidden="true">⌂</span>Home</Link>
          {items.map((item) => <Link href={item.href} key={item.href} onClick={() => setIsOpen(false)}>{item.label}{item.count ? <span className="count">{item.count}</span> : null}</Link>)}
        </nav>
        <div className="mobile-menu-footer"><p>{companyName}</p><button className="sign-out" type="button" onClick={closeAndSignOut}><span aria-hidden="true">↪</span> Log out</button></div>
      </aside>
    </>}
  </div>;
}
